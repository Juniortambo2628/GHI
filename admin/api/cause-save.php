...
// Get form data
$causeId = isset($_POST['id']) ? (int)$_POST['id'] : 0;
...
try {
    ...
    if ($causeId > 0) {
        ...
        log_message('info', 'Cause updated via modal', ['cause_id' => $causeId]);
    } else {
        // Create new
        ...
        log_message('info', 'Cause created via modal', ['cause_id' => $causeId]);
    }
    ...
    echo json_encode([
        'success' => true,
        'message' => $message,
        'data' => array_merge(['id' => $causeId], $data)
    ]);
    
} catch (\Exception $exception) {
    log_message('error', 'Cause save failed via modal', ['error' => $exception->getMessage()]);
