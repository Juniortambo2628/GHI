...
export default function Edit({ story, events = [] }) {
    ...
    const submit = e => {
        e.preventDefault();
        put(`/admin/stories/${story.id}`, { onSuccess: () => autosave.deleteDraft() });
    };
    ...
    return (
        <AdminEntityForm
            ...
            submit={submit}
            breadcrumbs={[{ label: 'Dashboard', href: '/admin' }, { label: 'Stories', href: '/admin/stories' }, { label: 'Edit' }]}
            formKey={`stories-edit-${story.id}`}
            autosave={autosave}
        >
