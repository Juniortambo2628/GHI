...
export default function Create({ events }) {
    ...
    const autosave = useAutosave({ formKey: 'impact-create', data, enabled: true });

    const submit = e => {
        e.preventDefault();
        post('/admin/impact', { onSuccess: () => autosave.deleteDraft() });
    };
    ...
    return (
        <AdminEntityForm
            ...
            submit={submit}
            breadcrumbs={[{ label: 'Dashboard', href: '/admin' }, { label: 'Impact', href: '/admin/impact' }, { label: 'Create' }]}
            formKey="impact-create"
            autosave={autosave}
        >
